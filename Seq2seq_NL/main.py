...
# model_id = model_id_dict[args.random_seed]
# RE_trainer.load_state_dict(args.checkpoint_directory + "/%s.model" % model_id)
# test_gold = json.load(open(args.test_json, "r"))
# result = RE_trainer.eval_model(data.test_loader, data.relational_alphabet, test_gold,
#                             log_fn=model_id, print_pred=False)


Gen_trainer = Trainer(model, data, args, args.Gen_epoch, args.Gen_start_eval, gen=True)
Gen_trainer.train_model()

# model_id = '03-06-12-26-46'
# Gen_trainer.load_state_dict(args.checkpoint_directory + "/%s.model" % model_id)
# test_gold = json.load(open(args.test_json, "r"))
# result = Gen_trainer.eval_model(data.test_loader, data.relational_alphabet, test_gold,
#                             log_fn=model_id, print_pred=True)
